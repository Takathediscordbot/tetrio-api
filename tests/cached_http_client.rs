...
fn test_ok_success_is_some<T, E: Debug>(result: Result<Arc<Packet<T>>, E>) {
...
fn test_ok_failure_is_some<T, E: Debug>(result: Result<Arc<Packet<T>>, E>) {
    let data = result.expect("Request failed");
    assert_eq!(!data.is_success(), true);
...
fn test_ok_success_is_none<T, E: Debug>(result: Result<Arc<Packet<T>>, E>) {
    let data = result.expect("Request failed");
    assert_eq!(data.is_success(), true);
    assert_eq!(data.data.is_none(), true);
    assert_eq!(data.cache.is_some(), true);
...
async fn fetch_leaderboard_country_query() {
    ...
    test_ok_success_is_some(
        client
            .fetch_league_leaderboard(ValueBoundQuery::NotBound {
                limit: None,
    ...
    test_ok_success_is_some(
        client
            .fetch_league_leaderboard(ValueBoundQuery::After {
                after: ordered_float::OrderedFloat(22000.5),
                limit: None,
                country: None,
                session_id: None
            })
...
async fn fetch_xp_leaderboard_country_query() {
    ...
    test_ok_success_is_some(client
        .fetch_xp_leaderboard(ValueBoundQuery::NotBound {
            limit: None,
            country: Some("fr".to_string()),
        })
        .await);
...
async fn fetch_full_league_leaderboard_country() {
    ...
    let client = CachedClient::default();
    test_ok_success_is_some(client
        .fetch_full_league_leaderboard(Some("fr"))
        .await);
    test_ok_success_is_some(client
        .fetch_full_league_leaderboard(Some("fr"))
        .await);
